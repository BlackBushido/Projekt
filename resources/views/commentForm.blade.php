@extends('layouts.app')
@section('title','Kreator komentarzy')
@section('content')
<div class="title"> <h3>Dodaj komentarz</h3> </div>
<div class="table-container bg-gradient">
    <!-- /.box-header -->
    <!-- form start -->
    <form  action="{{ route('comments.store',['id'=>$var]) }}" id="comment-form" method="post" enctype="multipart/form-data" >
        @csrf
        <div class="box border border-secondary py-1" >
            <div class="box-body" >
                <div class="form-group{{ $errors->has('message')?'has-error':'' }}" id="roles_box">
                    <div class="mb-3">
                        <textarea class="form-control" id="opis" name="opis" type="text" placeholder="Komentarz" style="height: 10rem; width: 50rem" data-sb-validations="required">{{old('opis')}}</textarea>
